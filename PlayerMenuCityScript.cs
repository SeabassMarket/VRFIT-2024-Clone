...
public class PlayerMenuCityScript : MonoBehaviour
{
    ...
    void Update()
    {
        if (VRInput.GetComponent<InputReader>().leftPrimarySwitch)
        {
            ...
            if (VRInput.GetComponent<InputReader>().menuButtonSwitch)
            {
                ...
                //Set location
                playerMenuXPosition = VRInput.GetComponent<InputReader>().headPosition.z * -1 + XRRig.transform.position.x + playerMenuDistance * Mathf.Sin((VRInput.GetComponent<InputReader>().eulerHeadRotation.y + XRRig.transform.rotation.eulerAngles.y) * Mathf.Deg2Rad);
                playerMenuYPosition = XRRig.transform.position.y + VRInput.GetComponent<InputReader>().headPosition.y;
                playerMenuZPosition = VRInput.GetComponent<InputReader>().headPosition.x + XRRig.transform.position.z + playerMenuDistance * Mathf.Cos((VRInput.GetComponent<InputReader>().eulerHeadRotation.y + XRRig.transform.rotation.eulerAngles.y) * Mathf.Deg2Rad);
